...
- `functions/` package holds the built-in-function *registration machinery* (not the tree-node types above, and not the function bodies themselves):
  ...
  - `FunctionRegistry` — reflects over an explicit, hand-maintained list of provider classes (`functions/builtin/*`), and for each `@MushFunction` method builds a `MethodHandle`/`LambdaMetafactory`-based adapter (not per-call `Method.invoke`). Non-lazy methods are adapted into an internal `Value`-based `ValueHandler`, then wrapped in a `MushFunctionHandler` that evaluates all `Expression` arguments before delegating; `lazy` methods are adapted straight to `MushFunctionHandler`, no wrapper. Generic arg-count validation and `MushValueException` → `#-1 ...` conversion are applied uniformly to both afterward. `FunctionRegistry.build()` returns the `Map<String, MushFunctionHandler>` `MushcodeParser`'s constructor expects.
  - `functions/builtin/` holds the provider classes with the actual function bodies, one class per category mirroring `functions.c` (e.g. `MathFunctions`: `add`/`sub`/`abs`; `RegisterFunctions`: `setq`; `ControlFunctions`: `switch`, `lazy = true`; `ObjectFunctions`: caller-identity stubs predating the registry, not yet migrated to `@MushFunction`).
