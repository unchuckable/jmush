...
/**
 * A minimal telnet client for driving a running TinyMUSH server (the compatibility oracle). Not
 * thread-safe; one client per session.
 */
public class OracleClient implements Closeable {
  ...
  /**
   * Evaluates a mushcode snippet via {@code think} and returns its raw output (CRLFs preserved
   * as-is), with exactly the one trailing CRLF that {@code notify()} itself appends stripped off.
   * Uses a unique sentinel command, read as raw characters (not line-by-line) to detect the end of
   * output without losing embedded line-ending information -- compatibility bugs live in exactly
   * that detail (e.g. mushcode's %r).
   */
