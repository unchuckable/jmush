 ...
 * {@code Map<String, MushFunctionHandler>} the parser expects, one entry per
 * {@code @MushFunction}-annotated method. Adapters are built once here via
 * {@code MethodHandle}/{@code LambdaMetafactory} (not per-call reflective {@code
 * Method.invoke}), so the natural-parameter style costs nothing at runtime once
 * JIT-warmed -- the same mechanism the JVM uses for ordinary lambdas/method references.
 * Generic arg-count validation and {@link MushValueException} -> {@code #-1 ...}
 * conversion are applied here too, so individual function bodies stay free of that
 * boilerplate.
 */
public class FunctionRegistry {

  private static final List<Class<?>> PROVIDER_CLASSES = Arrays.asList(
      MathFunctions.class
  );
  ...
  private static MushFunctionHandler buildHandler(
    ...
    if (paramTypes.length == 2 && paramTypes[1] == List.class) {
      ...
      throw new IllegalArgumentException(
          "@MushFunction method must take (ExecutionContext, Value...) or "
              + "(ExecutionContext, List<Value>): " + method);
    }
    ...
    return (ctx, args) -> {
      ...
      if (argCount < minArgs || argCount > maxArgs) {
        return Value.of("#-1 FUNCTION (" + name.toUpperCase() + ") EXPECTS "
            + expectedArgsDescription(minArgs, maxArgs) + " ARGUMENTS");
      }
      try {
  ...
  private static MushFunctionHandler adaptFixedArity(
    ...
    try {
      ...
      switch (arity) {
        case 1: {
          MushFunction1 fn = (MushFunction1) LambdaMetafactory.metafactory(
              lookup, "apply",
              MethodType.methodType(MushFunction1.class),
              MethodType.methodType(Value.class, ExecutionContext.class, Value.class),
              target, target.type()).getTarget().invoke();
          return (ctx, args) -> fn.apply(ctx, args.get(0));
        }
        case 2: {
          MushFunction2 fn = (MushFunction2) LambdaMetafactory.metafactory(
              lookup, "apply",
              MethodType.methodType(MushFunction2.class),
              MethodType.methodType(Value.class, ExecutionContext.class, Value.class, Value.class),
              target, target.type()).getTarget().invoke();
          return (ctx, args) -> fn.apply(ctx, args.get(0), args.get(1));
        }
        default:
          throw new IllegalArgumentException(
              "No arity-specific MushFunctionN interface for " + arity
